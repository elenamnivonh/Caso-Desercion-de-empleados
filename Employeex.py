...
employee_load = st.text('Loading employees data...')
employee = load_data(501)
employee_load.text("Done!")
...
hisplot_age= st.sidebar.checkbox("Histplot by Age")
if hisplot_age:
    fig, ax = plt.subplots()
...
city= st.sidebar.checkbox("City chart and Attrition rate")
if city:
    fig, ax = plt.subplots()
...
age= st.sidebar.checkbox("graphic: age and attrition rate")
if age:
    fig, ax = plt.subplots()
    y= employee["Attrition_rate"]
    x=employee["Age"]
...
service= st.sidebar.checkbox("Graphic: attrition rate and time of service")
if service:
    fig, ax = plt.subplots()
    y= employee["Attrition_rate"]
...
hometown= st.sidebar.text_input("Employee's Hometown")
search_hometown=st.sidebar.button("Search by hometown")

if(search_hometown):
    hometown_filter_if= df_hometown(hometown)
